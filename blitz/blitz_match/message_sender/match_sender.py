...
class MatchSender:
    ...
    async def start_match(self) -> None:
        ...
        for team in self.match_data.all_teams:
            ...
            message_photo = await self.sender.send_messages(
                text = text,
                characters = team.team.characters,
                photo = photo
            )
    ...
    async def send_event_scene(
        ...
        message_photo = await self.sender.send_messages(
            text = text_scene,
            characters = self.match_data.all_characters_in_teams,
            photo = photo
        )
        if message_photo and not is_save:
            await save_photo_id(
    ...
    async def send_ping_donate_energy(self, goal_time: int) -> None:
        chance_clubs = self.match_data.get_chance_teams()
    ...
    async def send_end_match(
        ...
        if winner_match_team:
            loser_club = self.match_data.get_opposite_team(
            ...
            text_match_info = self.getter_templates.format_message(
                ...
                extra_context = {
                    "winner_club_name": winner_match_team.team_name,
                    "loser_club_name":  loser_club.team_name,
                }
            )
            text = self.getter_templates.format_message(
                template = template,
